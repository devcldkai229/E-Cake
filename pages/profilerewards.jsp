...
<head>
    ...
    <title>Kaipizzas | Admin Dashboard</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <script type="module" src="https://unpkg.com/ionicons@7.1.0/dist/ionicons/ionicons.esm.js"></script>
    <script nomodule src="https://unpkg.com/ionicons@7.1.0/dist/ionicons/ionicons.js"></script>
    <script src="https://cdn.jsdelivr.net/gh/alpinejs/alpine@v2.x.x/dist/alpine.min.js" defer></script>
    <script src="js/profilerewards.js"></script>
</head>
<body>
                    ...
                    <div class="divide-y divide-gray-200 lg:grid lg:grid-cols-12 lg:divide-y-0 lg:divide-x">
                        <aside class="py-6 lg:col-span-3">
                            <nav class="space-y-1">
                                ...
                                <a href="/rewards"
                                   ...
                                   x-state-description="undefined: &quot;bg-teal-50 border-teal-500 text-teal-700 hover:bg-teal-50 hover:text-teal-700&quot;, undefined: &quot;border-transparent text-gray-900 hover:bg-gray-50 hover:text-gray-900&quot;">
                                    <ion-icon class="mr-2 text-3xl text-gray-700" name="gift-outline"></ion-icon>
                        ...
                        <div class="divide-y divide-gray-200 lg:col-span-9">
                            ...
                            <div class="px-4 py-6 sm:p-6 lg:pb-8">
                                <div>
                                    <h2 class="text-2xl font-medium leading-6 text-gray-900">Rewards</h2>
                                    <p class="mt-1 text-sm text-gray-600">This information will be displayed publicly so be careful what
                                        you share.</p>
                                    <p class="flex items-center mt-6 text-xl font-bold text-gray-600">Your points : <span class="text-xl font-bold text-gray-600" id="pointUserCurrent"> ${sessionScope.user.getPointRewards()}</span><ion-icon class="ml-2 text-xl font-light text-red-500" name="ribbon"></ion-icon></p>
                                </div>
                                <div class="grid grid-cols-1 gap-6 mt-8 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
                                    <c:forEach var="item" items="${rewardsList}">
                                        <div class="relative flex flex-col items-center justify-center w-full max-w-lg m-2 mx-auto overflow-hidden bg-white border border-gray-100 rounded-lg shadow-md">
                                            <a class="relative flex mx-3 mt-3 overflow-hidden h-60 rounded-xl" href="#">
                                                <input type="hidden" id="hiddenIdReward" value="${item.getId()}">
                                                <img class="object-cover" src="img/rewards/${item.getImageUrl()}" alt="product image" />
                                                <c:if test="${item.getQuantity()==0}">
                                                    <div class="absolute top-0 left-0 px-2 m-2 text-sm font-bold text-center text-red-500 bg-white rounded-full">Oversold</div>
                                                </c:if>
                                                <c:if test="${item.getQuantity()!=0}">
                                                    <div class="absolute top-0 left-0 px-2 m-2 text-sm font-bold text-center text-indigo-400 bg-white rounded-full">Special</div>
                                                </c:if>
                                            </a>
                                            <div class="px-5 pb-5 mt-4">
                                                <a href="#">
                                                    <h5 class="text-xl tracking-tight text-slate-900">${item.getName()}</h5>
                                                </a>
                                                <div class="flex items-center justify-between mt-2 mb-5">
                                                    <p class="flex items-center">
                                                            <span class="text-3xl font-bold text-slate-900">${item.getPoint()}<ion-icon class="ml-2 text-xl font-light text-red-500" name="ribbon"></ion-icon>
                                                            </span>
                                                    </p>
                                                    <p>
                                                        <span class="ml-2 text-sm text-slate-900">Units Of Stock: ${item.getQuantity()}</span>
                                                    </p>
                                                </div>
                                                <c:choose>
                                                    <c:when test="${item.getQuantity()!=0}">
                                                        <a onclick="exchangeRewardsProcessing(${item.getId()})" class="cursor-pointer flex items-center justify-center rounded-md bg-slate-900 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-blue-300">
                                                            <ion-icon class="mr-1 text-2xl font-bold " name="basket-outline"></ion-icon>
                                                            Exchange</a>
                                                    </c:when>
                                                    <c:when test="${item.getQuantity()==0}">
                                                        <a class="flex items-center justify-center rounded-md bg-slate-900 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-700 hover:text-red-400 focus:outline-none focus:ring-4 focus:ring-blue-300">
                                                            <ion-icon class="mr-1 text-2xl font-bold " name="basket-outline"></ion-icon>
                                                            Out Of Stock</a>
                                                    </c:when>
                                                </c:choose>
